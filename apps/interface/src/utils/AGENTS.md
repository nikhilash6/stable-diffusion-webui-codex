...
- 2026-05-23: `engine_taxonomy.ts` treats `zimage_l2p` as a first-class image tab/semantic/request id, separate from latent `zimage`. `image_request_contract.ts` is the canonical no-VAE L2P selector owner: it accepts exactly one `zimage_l2p/<path>` Qwen3-4B label resolved through `zimage_tenc`, requires `checkpoint_core_only=true` with `checkpoint|gguf` model format, and must not emit VAE selectors for L2P.
