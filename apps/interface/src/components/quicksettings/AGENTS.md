...
- 2026-05-23: `QuickSettingsBar.vue` renders `zimage_l2p` through `QuickSettingsAssetBlock.vue` with `showVae=false`, checkpoint roots from `zimage_l2p_ckpt`, and exactly one `zimage_l2p/<path>` Qwen3-4B selector sourced from `zimage_tenc`. L2P has no IMG2IMG/INPAINT/Turbo/VAE controls in the header.
