...
- 2026-05-18: `tabs/runtime.py` Engine view owns the `CODEX_CFG_BATCH_MODE` selector (`fused|split`, default `fused`); `tabs/diagnostics.py` no longer mirrors that runtime selector and instead exposes the explicit opt-in hot-path diagnostics `CODEX_VAE_TENSOR_STATS` and `CODEX_MEMORY_DEBUG`.
