...
- 2026-05-18: Launcher Engine owns `CODEX_CFG_BATCH_MODE=fused|split` (default `fused`) as a runtime behavior selector. Launcher Diagnostics owns opt-in hot-path diagnostic toggles for `CODEX_VAE_TENSOR_STATS` and `CODEX_MEMORY_DEBUG`; both default off.
