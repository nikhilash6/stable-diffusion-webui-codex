...
from .quant_shapes import quant_shape_to_byte_shape
...
class GGUFReader:
    ...
    def __init__(self, path: os.PathLike[str] | str, mode: Literal['r', 'r+', 'c'] = 'r'):
        ...
        if temp_version[0] & 65535 == 0:
            ...
            self.byte_order = 'S'
            temp_version = self._view_newbyteorder(temp_version, self.byte_order)
    ...
    def _get(
        ...
        return self._view_newbyteorder(arr, override_order or self.byte_order)

    @staticmethod
    def _view_newbyteorder(arr: npt.NDArray[Any], order: Literal['I', 'S', '<', '>', '='] | None) -> npt.NDArray[Any]:
        """Compat helper for NumPy >= 2.0 where ndarray.newbyteorder was removed.

        - order 'I' (native) maps to '='
        - order 'S' (swap) toggles current dtype endianness
        - orders '<'/'>'/'=' are forwarded directly to dtype.newbyteorder
        """
        if order is None or order == 'I' or order == '=':
            return arr.view(arr.dtype.newbyteorder('='))
        if order == 'S':
            # Toggle current endianness; for non-endian types ('|') this is a no-op
            try:
                return arr.view(arr.dtype.newbyteorder('S'))
            except Exception:
                return arr
        # '<' or '>' specified explicitly (or any other supported token)
