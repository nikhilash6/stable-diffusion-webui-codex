...
- 2026-05-18: GGUF mixed presets (`Q5_K_M` / `Q4_K_M`) no longer expose dtype selection. Profile-defined mixed/preserved float tensors keep the source floating dtype; official BF16 Qwen shards preserve those tensors as BF16 instead of upcasting them to F32, while ordinary compatible tensors remain quantized.
...
- 2026-01-15: Flux mixed presets (`Q5_K_M` / `Q4_K_M`) keep additional IO weights in the source floating dtype (larger GGUF than fully quantized, higher quality).
...
- 2026-01-15: GGUF converter keeps mixed/preserved tensors in source floating dtype through profile policy rules; user-selectable mixed float groups are not a live contract.
- 2026-01-16: Flux and WAN22 mixed dtype policy is profile-owned; Tools UI exposes only the Mixed preset toggle, not a mixed dtype selector.
...
- 2026-01-16: WAN22 GGUF converter has a mixed-preset dtype policy (Q*_K_M) keeping sensitive embedder/head weights in source floating dtype for quality.
- 2026-01-16: WAN22 converter presets expose only the profile-owned mixed policy; there is no Mixed AUTO/FP16/FP32 knob.
