...
- 2026-05-19: GGUF converter supports Qwen Image Qwen2.5-VL text-encoder weights through the `qwen_image_tenc` profile (`arch=qwen2_5_vl`), preserving native `model.*`/`visual.*` tensor names with no Llama key mapping and listing `text_encoder` components only when real SafeTensors shards are present.
...
- 2026-03-07: The converter emits source/native tensor names only for Flux, Qwen Image, Z-Image, WAN22, LTX-2, and Gemma3 text-encoder profiles; recipe/profile/required dtype rules target the tensor names that actually reach the GGUF writer.
