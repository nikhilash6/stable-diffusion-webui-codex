...
- `apps/backend/runtime/tools/gguf_converter_k_underflow.py` — Pre-header Q4_K/Q5_K stored-scale underflow scanner and tensor promotion policy for material affected tensors.
...
- 2026-05-24: Q4_K/Q5_K conversions run a pre-header stored-scale-underflow scan. Materially affected tensors are promoted to float storage before tensor info/header emission; non-material affected tensors stay K-quantized with operator-visible warnings. The block predicate remains owned by `apps/backend/quantization/kernels/quantize_numpy.py`.
