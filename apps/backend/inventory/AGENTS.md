...
- 2025-12-04: Text encoder inventory uses per-engine roots from `apps/paths.json` (`sd15_tenc`, `sdxl_tenc`, `flux1_tenc`, `flux2_tenc`, `qwen_image_tenc`, `wan22_tenc`, `zimage_tenc`), deduplicating by path so engine-specific TEnc folders show up once under `/api/models/inventory`.
- 2025-12-04: Inventory also appends engine-specific VAEs from `apps/paths.json` (`flux1_vae`, `flux2_vae`, `qwen_image_vae`, `zimage_vae`) so split-asset image-family dropdowns can list concrete files without filename heuristics.
...
- 2026-05-17: Inventory discovery includes Qwen Image roots (`qwen_image_tenc`, `qwen_image_vae`) and preserves Qwen2.5-VL slot classification as `qwen2_5_vl_7b`.
